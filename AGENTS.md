...
Build the shared YourOwn Platform, not a product backend. GCP, AWS, Azure and Cloudflare are first-class integration targets. Use Go for first-party executable tools and MCP adapters. Terraform remains the infrastructure engine; do not build a replacement orchestration language.

## Non-negotiable boundaries

- Never commit credentials, real IP/CIDR literals, account/billing/project IDs, Azure tenant/subscription/client/principal IDs, deployed origins, private inventories, state, plans, production renders or kubeconfig files.
...
Keep cloud semantics explicit: Cloud Run, ECS and Azure Container Apps are not identical runtimes; GKE, EKS and AKS also retain provider-specific semantics. Kubernetes, NAT, HA databases, HSM and cross-cloud networking are opt-in capabilities, not mandatory baseline costs.
